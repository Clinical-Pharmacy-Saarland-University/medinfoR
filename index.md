...
It provides drug interaction checks, adverse drug reactions (ADRs),
pharmacogenetic guidelines, PRISCUS classifications, QT prolongation
risk, product information, and more.
...
New accounts receive an initial token by email. Use it to set your
password:
...
returns an `ApiCredentials` object that is passed to every API function.
...
Integration tests call the live API. They are skipped automatically
unless the required environment variables are set. Create a `.env` file
in the package root:
